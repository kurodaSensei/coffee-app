...
import type { CoffeeInput, CoffeeProcess, RoastLevel } from '~/types'
...
function validateStep1(): boolean {
  ...
  if (!roasterName.value.trim()) errors.value.roasterName = 'El tostador es obligatorio'
...
async function submit() {
  ...
  try {
    const payload: CoffeeInput = {
      ...
      roasterName: roasterName.value.trim() || undefined,
          ...
          <div class="flex flex-col gap-xs pt-[14px] pb-[13px] border-b border-moss/10">
            <div class="flex items-baseline justify-between gap-md">
              <label for="altitude" class="font-mono text-[10px] font-medium uppercase tracking-eyebrow text-moss-soft">
                <span aria-hidden="true">— </span>Altitud <span class="lowercase normal-case font-display italic text-moss-soft">opcional</span>
              </label>
